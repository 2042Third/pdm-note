...
void cMain::init_pane_files_files() {
  ...
  pane_files = new wxRichTextCtrl(panel_files_files, wxEVT_DROP_FILES, _T(""), wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxTE_READONLY);
    ...
    usr_enter = new wxTextCtrl(panel_files_files,wxEVT_TEXT_ENTER, wxEmptyString,
                               ...
                               "密码");
    usr_enter_nm_text= new wxStaticText(panel_files_files, wxID_ANY, _("User"));
    usr_enter_nm = new wxTextCtrl(panel_files_files,wxEVT_TEXT_ENTER, wxEmptyString,
                               wxDefaultPosition,  wxDefaultSize,
                               0,  wxDefaultValidator,
                               "User");
    passwd_sizer->Add(pswd_text);
    passwd_sizer->Add(usr_enter);
    passwd_sizer->Add(usr_enter_nm_text);
    passwd_sizer->Add(usr_enter_nm);
...
void cMain::stc_save_as(wxCommandEvent& event){
  ...
  CurrentFileName = "new_file";
  wxString ur_name = tctl_to_ary(usr_enter_nm);
  if(ur_name.empty()) ur_name="no_user";
  saved_pdm_dir->Mkdir(saved_file_dir+sp+ur_name,0777,wxPATH_MKDIR_FULL);
  OpenDialog = new wxFileDialog(
      this, _("Save As file"), saved_file_dir+sp+ur_name, CurrentFileName,
      _("*"),
...
void cMain::maintain_theme(){
  ...
  usr_enter_nm_text->SetForegroundColour(wxTheColourDatabase->Find("WHITE"));
  ...
  usr_enter_nm->SetBackgroundColour(wxTheColourDatabase->Find("DARK GREY"));
  usr_enter_nm->SetForegroundColour(wxTheColourDatabase->Find("WHITE"));
...
void cMain::stc_save(wxCommandEvent& event) {
    std::cout<<"[stc_save] Starting operation\n"<<std::endl;
  size_t inp_len=0;
      ...
      if (!usr_enter_nm->GetValue().empty()){//repeated in the function
        ...
        int sync_out = syncing->usr_sync(std::string(CurrentDocPathEnc.mb_str()));
          if(sync_out){
              ...
              tree_ctrl->d_target->m_pOwner->WriteText(_("Failed to save to cloud.\n"));
          }
      }
      else{
          tree_ctrl->d_target->m_pOwner->WriteText(_("No user name/account, not saved to cloud.\n"));
      }
    }
  ...
  saved_pdm_dir->Mkdir(saved_file_dir+sp+ur_name,0777,wxPATH_MKDIR_FULL);
  OpenDialog = new wxFileDialog(
      this, _("Save file"), saved_file_dir+sp+ur_name, CurrentFileName,
      _("*"),
      wxFD_SAVE, wxDefaultPosition, wxSize(300,500));
  if (OpenDialog->ShowModal() == wxID_OK ) // if the user click "Open" instead of "cancel"
    {
...
char* cMain::tctl_to_ary(wxTextCtrl* a){
    wxCharBuffer tta = a->GetValue().To8BitData();
    return tta.data();
}
//cMain::~cMain()
//{
