...
void Tree_Ctrl::try_get_from_cloud(wxTreeItemId a, wxString b){
  ...
  if(!wxDirExists(wxPathOnly(b))){
    b = file_directory+sp+((cMain*)parent)->tctl_to_ary(((cMain*)parent)->usr_enter_nm)+sp+ wxFileNameFromPath(b);
    saved_dir->Mkdir(b   ,0777,wxPATH_MKDIR_FULL);
  }
    ((cMain*)parent)->syncing->usr_get(
      (wxFileNameFromPath(b).ToStdString()),
      static_cast<std::string>(wxPathOnly(b) + sp + wxFileNameFromPath(b).ToStdString()));
  if(wxFileExists(b))
