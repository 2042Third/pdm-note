...
//  wxString directory_name=pdm_dir->GetCwd()+sp+"pdm_rc.conf";
  ...
  while(load_rc2){
    if(load_rc2->GetName()=="logs"){
      ...
      directory_name+sp+file_name+ " at "+
      load_rc1->GetContent());
    }
...
void pdmrc::read_rc(){
  ...
  //User name save
  delete user_rc;
  user_rc=new wxXmlNode(wxXML_ELEMENT_NODE,"last-user");
  count=0;
  tree_file_rc=NULL;
  tree_str1_rc=NULL;
  tree_str2_rc=NULL;
  tree_file_rc=new wxXmlNode(wxXML_ELEMENT_NODE,"u_nm"+std::to_string(count));
  tree_str2_rc=new wxXmlNode(wxXML_TEXT_NODE,"",((cMain*)parent)->usr_enter_nm->GetValue());
  tree_file_rc->AddChild(tree_str2_rc);
  user_rc->AddChild(tree_file_rc);
}
